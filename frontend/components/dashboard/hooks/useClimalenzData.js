...
export function useClimalenzData(initialCityId = 'mumbai') {
  ...
  // CoLocation Analysis State
  const [colocationState, setColocationState] = useState('IDLE'); // IDLE | ANALYZING | SUCCESS | ERROR
  const [colocationResult, setColocationResult] = useState(null);
  const [colocationError, setColocationError] = useState(null);
